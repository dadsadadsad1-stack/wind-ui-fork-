...
function Element:New(Config)
    ...
    if Section.Badge then
        BadgeFrame = createBadge(Section.Badge)
    ...
    local function UpdateTitleSize()
        ...
        if BadgeFrame then
            offset = offset - (BadgeFrame.AbsoluteSize.X + 8)
        end
    ...
    -- Section.UIElements.Main:GetPropertyChangedSignal("TextBounds"):Connect(function()
        ...
        function Section:SetBadge(Badge)
            Section.Badge = Badge
            if BadgeFrame then
                BadgeFrame:Destroy()
            end
